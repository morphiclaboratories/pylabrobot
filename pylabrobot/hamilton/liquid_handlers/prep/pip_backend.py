"""Hamilton Prep PIP backend.

:class:`PrepPIPBackend` takes narrow dependencies at construction — the
:class:`PrepDriver` (transport + JIT firmware-path resolve) and
:class:`PrepInstrumentInfo` (instrument-wide metadata). It holds no backref
to :class:`Prep`. Per-channel topology lives in :mod:`.channels`; ``self.channels``
is populated from :meth:`Prep.setup` before :meth:`_on_setup`.
"""
...
@dataclass
class PrepPIPPickUpTipsParams(BackendParams):
...
@dataclass
class PrepPIPDropTipsParams(BackendParams):
  final_z: Optional[float] = None
  seek_speed: float = 15.0
  z_seek_offset: Optional[float] = None
...
@dataclass
class PrepPIPAspirateParams(BackendParams):
...
@dataclass
class PrepPIPDispenseParams(BackendParams):
  z_final: Optional[List[float]] = None
  z_fluid: Optional[List[float]] = None
  z_air: Optional[List[float]] = None
  settling_time: Optional[List[float]] = None
...
# =============================================================================
# PrepPIPBackend — channel indices and deck routing
# =============================================================================
...
class PrepPIPBackend(PIPBackend):
  """PIP backend for Hamilton Prep.

  Narrow constructor: ``driver`` (transport + JIT firmware-path resolve) and
  ...
  async def _on_setup(self, backend_params: Optional[BackendParams] = None):
    ...
    Called from :class:`~pylabrobot.capabilities.liquid_handling.pip.PIP`
    after ``self.channels`` is populated by :meth:`Prep.setup`. Instrument-
    ...
    """
    del backend_params  # accepted for PIPBackend contract; unused
    ...
    if self._use_v1_aspirate_dispense:
      ...
      if not supported:
        raise RuntimeError(
          "V2 aspirate/dispense commands (cmd 38-43) are not supported by this firmware. "
          "Pass use_v1_aspirate_dispense=True to PrepPIPBackend to use v1 commands (cmd 1-6) instead."
        )
  ...
  async def discover_channel_drives(self) -> ChannelDriveMap:
    ...
    """
    return await discover_channel_drives(self._driver, root_name="Channel Root")
  ...
  def _resolve_traverse_height(self, final_z: Optional[float] = None) -> float:
    ...
    raise RuntimeError(
      ...
      "PrepPIPBackend(..., default_traverse_height=<mm>) or backend.set_default_traverse_height(<mm>). "
      "If the instrument supports it, the value is also probed during setup(); ensure setup() completed successfully."
    ) from None

  # ---------------------------------------------------------------------------
  # PIPBackend + extended Prep API
  # ---------------------------------------------------------------------------

  async def pick_up_tips(
    self, ops: List[Pickup], use_channels: List[int], backend_params: Optional[BackendParams] = None
  ):
    """Pick up tips.
    ...
    Args:
      backend_params: Optional :class:`PrepPIPPickUpTipsParams` for ``final_z``,
        ``seek_speed``, ``z_seek_offset``, TADM fields, etc.
    """
    p = (
      backend_params
      if isinstance(backend_params, PrepPIPPickUpTipsParams)
      else PrepPIPPickUpTipsParams()
    )
    final_z = p.final_z
  ...
  async def drop_tips(
    ...
    Args:
      backend_params: Optional :class:`PrepPIPDropTipsParams` for ``final_z``,
        ``seek_speed``, ``drop_type``, etc.
    """
    p = (
      backend_params
      if isinstance(backend_params, PrepPIPDropTipsParams)
      else PrepPIPDropTipsParams()
    )
    final_z = p.final_z
    seek_speed = p.seek_speed
    z_seek_offset = p.z_seek_offset
    drop_type = p.drop_type
    tip_roll_off_distance = p.tip_roll_off_distance
  ...
  async def aspirate(
    ...
    Args:
      ops: :class:`~pylabrobot.capabilities.liquid_handling.standard.Aspiration` ops
        (``mix`` uses :class:`~pylabrobot.capabilities.liquid_handling.standard.Mix`).
      backend_params: Optional :class:`PrepPIPAspirateParams` for Z/LLD/TADM and related fields.
    ...
    p = (
      backend_params
      if isinstance(backend_params, PrepPIPAspirateParams)
      else PrepPIPAspirateParams()
    )
    z_final = p.z_final
    z_fluid = p.z_fluid
    z_air = p.z_air
    settling_time = p.settling_time
    transport_air_volume = p.transport_air_volume
    z_liquid_exit_speed = p.z_liquid_exit_speed
    prewet_volume = p.prewet_volume
    z_minimum = p.z_minimum
    z_bottom_search_offset = p.z_bottom_search_offset
    lld_mode = p.lld_mode
    lld = p.lld
    p_lld = p.p_lld
    c_lld = p.c_lld
    tadm = p.tadm
    container_segments = p.container_segments
    auto_container_geometry = p.auto_container_geometry
    hamilton_liquid_classes = p.hamilton_liquid_classes
    disable_volume_correction = p.disable_volume_correction
  ...
  async def dispense(
    ...
    Args:
      ops: :class:`~pylabrobot.capabilities.liquid_handling.standard.Dispense` ops.
      backend_params: Optional :class:`PrepPIPDispenseParams`.
    ...
    p = (
      backend_params
      if isinstance(backend_params, PrepPIPDispenseParams)
      else PrepPIPDispenseParams()
    )
    z_final = p.z_final
    z_fluid = p.z_fluid
    z_air = p.z_air
    ...
    transport_air_volume = p.transport_air_volume
    z_liquid_exit_speed = p.z_liquid_exit_speed
    stop_back_volume = p.stop_back_volume
    cutoff_speed = p.cutoff_speed
    z_minimum = p.z_minimum
    z_bottom_search_offset = p.z_bottom_search_offset
    lld_mode = p.lld_mode
    lld = p.lld
  ...
  # ---------------------------------------------------------------------------
  # Firmware version queries (per-channel; box-level queries live on PrepDriver)
  # ---------------------------------------------------------------------------
  ...
  async def request_channel_bounds(self) -> list[dict]:
    ...
    """
    return await request_channel_bounds(self._driver)
  ...
  async def clld_probe_x_position_using_channel(self, *args, **kwargs):
    ...
    raise NotImplementedError(
      "clld_probe_x_position_using_channel is not yet implemented for PrepPIPBackend."
    )
  ...
  async def clld_probe_y_position_using_channel(self, *args, **kwargs):
    ...
    """
    raise NotImplementedError(
      "clld_probe_y_position_using_channel is not yet implemented for PrepPIPBackend."
    )
  ...
  async def clld_probe_z_height_using_channel(self, *args, **kwargs):
    ...
    """
    raise NotImplementedError(
      "clld_probe_z_height_using_channel is not yet implemented for PrepPIPBackend."
    )
  ...
  async def ztouch_probe_z_height_using_channel(self, *args, **kwargs):
    ...
    """
    raise NotImplementedError(
      "ztouch_probe_z_height_using_channel is not yet implemented for PrepPIPBackend."
    )
  ...
  async def move_channels_to_safe_z(self, channels: Optional[List[int]] = None) -> None:
    ...
    await self._driver.send_command(PrepCmd.PrepMoveZUpToSafe(channels=channel_enums))
